...
GP_PATH="${INDEX_PREFIX_PATH}GP_TIMES_${GP_TIMES}_LOCK_${GP_LOCK_NUMS}_GP_USE_FREQ${GP_USE_FREQ}/"
  ...
  gp)
    ...
    if [ ${GP_USE_FREQ} -eq 1 ]; then
      time ${EXE_PATH}/graph_partition/partitioner -D $DATA_DIM -N $DATA_N --index_file ${OLD_INDEX_FILE} \
        --data_type $DATA_TYPE --gp_file $GP_FILE_PATH -T $GP_T --ldg_times $GP_TIMES --freq_file ${FREQ_PATH}_freq.bin --lock_nums ${GP_LOCK_NUMS} > ${GP_FILE_PATH}.log
    else
      time ${EXE_PATH}/graph_partition/partitioner -D $DATA_DIM -N $DATA_N --index_file ${OLD_INDEX_FILE} \
