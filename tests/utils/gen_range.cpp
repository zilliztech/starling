...
template<typename T>
int aux_main(const std::string &base_file, const std::string &query_file, const std::string & radius_file, const std::string& k_file,
  ...
  unsigned k_npts =0 ;
  auto k_file_reader = std::ifstream(k_file);
  k_file_reader.read((char*)&k_npts, sizeof(unsigned));
  if(k_npts != nqueries){
    std::cout << "k file length not match query vector num" << std::endl;
    exit(-1);
  }
  std::vector<unsigned> k_vec(k_npts);
  k_file_reader.read((char*)k_vec.data(), sizeof(unsigned)*k_npts);

  std::vector<float> radius_vec(k_npts);
  ...
  auto gt_writer = std::ofstream(gt_file);
  std::vector<std::vector<unsigned>>res_ids(k_npts);
#pragma omp parallel for
  for (_u64 i = 0; i < nqueries; i++) {
    ...
    while(q_k < k && q_k > 0 && cur_res[q_k - 1].second == cur_res[q_k].second){
      q_k++;
    }
    for(_u64 j=0; j< q_k; j++){
      res_ids[i].push_back(cur_res[j].first);
    }
    // for (_u64 j = 0; j < q_k; j++) {
    //   gt_writer.write((char*)&cur_res[j].first, sizeof(unsigned));      
    // }
    radius_vec[i] = q_k==0?(cur_res[0].second+0)/2:(cur_res[q_k-1].second + cur_res[q_k].second)/2;
  }
  unsigned res_total = 0;
  for(auto n: k_vec){
    res_total+=n;
  }
  gt_writer.write((char*)&k_npts, sizeof(unsigned));
  gt_writer.write((char*)&res_total, sizeof(unsigned));
  gt_writer.write((char*)k_vec.data(), k_npts *sizeof(unsigned));
  for(auto &res:res_ids){
    gt_writer.write((char*)res.data(), res.size()*sizeof(unsigned));
  }
  save_bin(radius_file, radius_vec.data(), k_npts, 1);
  diskann::aligned_free(query_data);
  return 0;
}

int main(int argc, char **argv) {
  std::string data_type, dist_fn, base_file, query_file, gt_file, radius_file, k_file;
  uint64_t    K;
  ...
  try {
    ...
    desc.add_options()(
        "radius_file", po::value<std::string>(&radius_file)->required(),
        "File name for query radius writed in binary format");
    desc.add_options()(
        "k_file", po::value<std::string>(&k_file)->required(),
        "File containing the query's topk, readed in binary format");
...
try {
  if (data_type == std::string("float"))
    aux_main<float>(base_file, query_file,radius_file,k_file, gt_file, K, metric);
  if (data_type == std::string("int8"))
    aux_main<int8_t>(base_file, query_file,radius_file, k_file, gt_file, K, metric);
  if (data_type == std::string("uint8"))
    aux_main<uint8_t>(base_file, query_file,radius_file,k_file, gt_file, K, metric);
}
