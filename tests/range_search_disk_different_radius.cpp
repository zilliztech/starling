...
int search_disk_index(diskann::Metric&   metric,
  ...
  int radius_num, radius_dim;
  diskann::load_aligned_bin<T>(query_file, query, query_num, query_dim,
                               query_aligned_dim);

  std::ifstream radius_reader(radius_file);
  radius_reader.read((char*)&radius_num, sizeof(int));
  radius_reader.read((char*)&radius_dim, sizeof(int));
  if((size_t)radius_num != query_num){
    diskann::cout << "radius file not match" << std::endl;
  }
  std::vector<float> radius_vec = std::vector<float>(radius_num);
  radius_reader.read((char*)radius_vec.data(), sizeof(float)*query_num);
  radius_reader.close();
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  std::string recall_string = "Recall" ;
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    desc.add_options()("radius_file",
                       po::value<std::string>(&radius_file)->required(),
                       "radius file in binary format");
    desc.add_options()(
        "gt_file",
